...
async def execute_deployment_process(job_id: str, config: DeploymentConfig):
    ...
    try:
        ...
        config_dict = {
            ...
            "ubuntu_host": config.ubuntu_host if config else "10.138.77.157",
...
async def test_deployment_connection():
    ...
    try:
        ...
        ssh.connect("10.138.77.157", username="tcs", password="tcs@12345", timeout=10)
        ssh.close()
        
        return {
            "success": True,
            "message": "Connection test successful",
            "server": "10.138.77.157",
            "user": "tcs"
        }
...
async def execute_test_execution_process(job_id: str, config: TestExecutionConfig):
    """Background task for test execution (runs jenkins_build_trigger_with_rtesults.py as subprocess)."""
    try:
        ...
        # Get the path to jenkins_build_trigger_with_rtesults.py script
        current_dir = Path(__file__).parent.parent
...
class ExecuteCommandsRequest(BaseModel):
    """Request model for executing provided terminal commands"""
...
@router.post("/api/error-fixing/execute-commands")
async def execute_error_fixing_commands(request: ExecuteCommandsRequest):
    """Execute explicitly provided terminal commands for dependency/version fixes."""
    try:
        terminal_commands: List[Dict[str, Any]] = []
        for cmd in request.commands or []:
            if isinstance(cmd, dict):
                command_text = str(cmd.get("command") or cmd.get("text") or "").strip()
                if command_text:
                    terminal_commands.append({
                        "command": command_text,
                        "explanation": str(cmd.get("explanation") or cmd.get("hint") or "LLM suggested command")
                    })
        ...
        print(f"Error executing error-fixing commands: {e}")
        import traceback
        traceback.print_exc()
